...
/************************************************************************************************************************************/
/***** Drag & Drop RBM **************/
function RbmElementFromPoint(x, y, docObj,docWindow) {
    var check = false, isRelative = true;

    
    if (!docObj.elementFromPoint) return null;

        if (!check) {
            var sl;
            if ((sl = $(docObj).scrollTop()) > 0) {
                isRelative = (docObj.elementFromPoint(0, sl + $(docWindow).height() - 1) == null);
            }
            else if ((sl = $(docObj).scrollLeft()) > 0) {
                isRelative = (docObj.elementFromPoint(sl + $(docWindow).width() - 1, 0) == null);
            }
            check = (sl > 0);
        }
        ...
        return docObj.elementFromPoint(x, y);
}
